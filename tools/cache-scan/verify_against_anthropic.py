...
def main():
    ...
    P("    %-20s %14s %14s %14s %13s"
      % ("model", "real$ (Src1)", "SPLIT$", "FLAT$", "split vs real"))
    P("    " + "-" * 78)
    t_real = t_split = t_flat = 0.0
    for nm in order:
        l = local.get(nm, {})
        sp = l.get("split", 0.0); fl = l.get("flat", 0.0)
        real = cost["permodel"].get(nm) if cost else None
        t_split += sp; t_flat += fl
        ...
        if sp == 0 and fl == 0 and (real is None or real == 0):
            continue
        P("    %-20s %14s %14s %14s %13s"
          % (nm[:20], real_s, money(sp), money(fl), diff))
    P("    " + "-" * 78)
    real_tot_s = money(t_real) if cost else "   (n/a)"
    P("    %-20s %14s %14s %14s %13s"
      % ("TOTAL", real_tot_s, money(t_split), money(t_flat),
         pct(t_split, t_real) if cost else "   n/a"))
    if cost:
        ...
        if nontok:
            P("    note: Src1 real total includes $%s of non-token line items "
              ...
              % money(nontok))
        P("    note: SPLIT honors the 1h/5m cache-creation split; FLAT bills all "
          "cache-create @5m (what buggy ccusage reports).")
    ...
    if args.out:
        out = {
            ...
            "totals": {"real": t_real if cost else None,
