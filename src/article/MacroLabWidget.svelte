<script lang="ts">
  ...
  export type MacroLabHighlight = "all" | "strategy" | "model" | "effort" | "volume" | "compaction";

  interface Props {
    highlight?: MacroLabHighlight;
  }

  let { highlight = "all" }: Props = $props();
  ...
  const focused = $derived(highlight !== "all");
  const result = $derived(macroLabComparison({ strategy, model, effort, taskCount, contextDropped }));
  const baseline = macroLabComparison({ strategy: "uniform", model: "opus", effort: "high", taskCount: 7, contextDropped: 0 });
  const heldAt = $derived.by(() => {
    if (highlight === "strategy") return "model opus · high · 7 tasks/day · 0% compaction";
    if (highlight === "model") return "strategy uniform · high · 7 tasks/day · 0% compaction";
    if (highlight === "effort") return "strategy uniform · model opus · 7 tasks/day · 0% compaction";
    if (highlight === "volume") return "strategy uniform · model opus · high · 0% compaction";
    return "strategy uniform · model opus · high · 7 tasks/day";
  });
  const volumeId = $derived(highlight === "all" ? "macrolab-volume" : `macrolab-volume-${highlight}`);
  const compactionId = $derived(highlight === "all" ? "macrolab-compaction" : `macrolab-compaction-${highlight}`);
...
<article class:focused class="macrolab toycard" data-testid="macro-lab-widget" data-highlight={highlight}>
  ...
  {#if !focused}
    ...
    <div class="sliders">
      <label for={volumeId}><span>Tasks / day</span><strong class="toy-num">{taskCount}</strong></label>
      <input id={volumeId} class="toy-range" type="range" min="1" max="21" bind:value={taskCount} />
      <label for={compactionId}><span>Context dropped by compaction</span><strong class="toy-num">{contextDropped}%</strong></label>
      <input id={compactionId} class="toy-range" type="range" min="0" max="80" step="10" bind:value={contextDropped} />
    </div>
  {:else}
    <div class="focus-controls">
      {#if highlight === "strategy"}
        <div class="focus-control" data-testid="macrolab-control-strategy">
          <small>Move one parameter</small>
          <div class="strategy toy-seg" aria-label="Routing strategy">
            <button class:active={strategy === "uniform"} aria-pressed={strategy === "uniform"} onclick={() => strategy = "uniform"}>All through one model+effort</button>
            <button class:active={strategy === "routed"} aria-pressed={strategy === "routed"} onclick={() => strategy = "routed"}>Right-sized</button>
          </div>
        </div>
      {:else if highlight === "model"}
        <label class="focus-control" data-testid="macrolab-control-model">Move one parameter · model
          <select class="toy-select" bind:value={model} aria-label="Uniform model">
            <option value="haiku">Haiku</option><option value="sonnet">Sonnet</option><option value="opus">Opus</option><option value="fable">Fable</option>
          </select>
        </label>
      {:else if highlight === "effort"}
        <label class="focus-control" data-testid="macrolab-control-effort">Move one parameter · effort
          <select class="toy-select" bind:value={effort} aria-label="Uniform effort">
            <option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option>
          </select>
        </label>
      {:else if highlight === "volume"}
        <div class="focus-control range-control" data-testid="macrolab-control-volume">
          <label for={volumeId}><span>Move one parameter · tasks / day</span><strong class="toy-num">{taskCount}</strong></label>
          <input id={volumeId} class="toy-range" type="range" min="1" max="21" bind:value={taskCount} />
        </div>
      {:else}
        <div class="focus-control range-control" data-testid="macrolab-control-compaction">
          <label for={compactionId}><span>Move one parameter · context dropped</span><strong class="toy-num">{contextDropped}%</strong></label>
          <input id={compactionId} class="toy-range" type="range" min="0" max="80" step="10" bind:value={contextDropped} />
        </div>
      {/if}
      <p class="held-at"><b>Held at:</b> {heldAt}</p>
    </div>
  {/if}

  {#if !focused || highlight === "strategy"}
    <div class="summary" class:focused-readout={focused}>
  ...
  {:else if highlight === "compaction"}
    <div class="summary focused-readout">
      ...
      <b>vs</b>
      <div><span>With {contextDropped}% compaction</span><strong class="toy-num">{money(result.selected.totalUsd)}</strong></div>
      <div class="toy-delta"><span>compaction saves</span><strong class="toy-num">{money(result.compactionSavedUsd)}</strong></div>
    </div>
  {:else}
    <div class="summary focused-readout">
      <div><span>{highlight === "model" ? "Opus baseline" : highlight === "effort" ? "High-effort baseline" : "7-task baseline"}</span><strong class="toy-num">{money(baseline.uniform.totalUsd)}</strong></div>
      <b>vs</b>
      <div><span>{highlight === "model" ? `${model} uniform` : highlight === "effort" ? `${effort} effort` : `${taskCount} tasks / day`}</span><strong class="toy-num">{money(result.uniform.totalUsd)}</strong></div>
      <div class="toy-delta"><span>vs baseline</span><strong class="toy-num">{money(Math.abs(result.uniform.totalUsd - baseline.uniform.totalUsd))} {result.uniform.totalUsd <= baseline.uniform.totalUsd ? "less" : "more"}</strong></div>
    </div>
  {/if}
  ...
  {:else}
    <footer class="focused-total">
      <span>Selected total</span><strong class="big toy-num" data-testid="macrolab-total" data-value={result.selected.totalUsd}>{money(result.selected.totalUsd)}</strong>
    </footer>
  {/if}
...
<style>
  ...
  .focus-controls{display:grid;grid-template-columns:minmax(0,1fr) auto;align-items:center;gap:var(--sp-3);padding:14px 18px}.focus-control{display:grid;gap:7px;min-width:0;padding:10px 12px;border:2px solid var(--toy-blue);border-radius:12px;background:var(--toy-blue-soft);box-shadow:3px 3px 0 var(--toy-border);font-size:.72rem;font-weight:900;letter-spacing:.04em;text-transform:uppercase}.focus-control>.strategy{width:max-content;max-width:100%}.focus-control select{width:min(220px,100%);text-transform:capitalize}.range-control label{display:flex;justify-content:space-between;gap:var(--sp-3)}.range-control input{width:100%}.held-at{margin:0;color:var(--toy-muted);font-size:.72rem;line-height:1.45}.held-at b{color:var(--toy-ink);text-transform:uppercase}.focused-readout{grid-template-columns:1fr auto 1fr auto}.focused .breakdown{padding-block:12px}.focused .breakdown>.toy-bar{height:42px}.focused-total{display:flex;align-items:center;justify-content:flex-end;gap:var(--sp-3);padding:10px 18px;border-top:1px dashed var(--toy-dash);background:var(--toy-cream)}.focused-total span{font-size:.68rem;letter-spacing:.05em;text-transform:uppercase}
