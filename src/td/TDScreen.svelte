  ...
  {:else}
    ...
    <div class="game-grid">
      ...
      <aside class="router">
        ...
        <div class="worker-selects"><label>Model<select aria-label="Main agent model" bind:value={control.main.model}>{#each MODELS as model}<option value={model}>{model}</option>{/each}</select></label><label>Effort<select aria-label="Main agent effort" bind:value={control.main.effort}>{#each EFFORTS as effort}<option value={effort}>{effort}</option>{/each}</select></label></div>
        <small class="rate-note">${MODEL_IN[control.main.model]}/M input · HUD rate changes immediately</small>
        <label class="sub-toggle"><input type="checkbox" bind:checked={control.useSubagents}><span><b>Use subagents</b><small>Fresh scoped context, routed by type</small></span></label>
        {#if control.useSubagents}
          <div class="routes"><div class="route-head"><b>TASK TYPE</b><b>MODEL</b><b>EFFORT</b></div>{#each ROUTABLE_TASK_TYPES as type}<div class="route-row"><span title={FIT_TABLE[type].label}>{typeMeta[type].icon} {type}</span><select aria-label={`${type} model`} bind:value={control.routes[type].model}><option value="inherit">inherit ({control.main.model})</option>{#each MODELS as model}<option value={model}>{model}</option>{/each}</select><select aria-label={`${type} effort`} bind:value={control.routes[type].effort}><option value="inherit">inherit ({control.main.effort})</option>{#each EFFORTS as effort}<option value={effort}>{effort}</option>{/each}</select></div><small class="fit">ideal: {FIT_TABLE[type].label}</small>{/each}</div>
