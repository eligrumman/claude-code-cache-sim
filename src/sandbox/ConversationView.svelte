<script lang="ts">
  ...
  const bucketOrder: BucketId[] = ["input", "cacheWrite", "cacheRead", "output", "keepWarm", "compaction"];
  const bucketColors: Record<BucketId, string> = {
    input: "#aaa9a2", cacheWrite: "#e9573f", cacheRead: "#3cab6d",
    output: "#3e83d5", keepWarm: "#8bd8aa", compaction: "#c47ee8",
  };
    ...
    {#if selected && tab === "cost"}
      <aside class="inspector">
        ...
        <dl>
          ...
          {#if selected.buckets.compaction.tokens}<dt>Auto-compact (Haiku)</dt><dd>{selected.buckets.compaction.tokens.toLocaleString()} tok</dd><dd>{money(selected.buckets.compaction.usd)}</dd>{/if}
        </dl>
      </aside>
    {/if}
  </div>
