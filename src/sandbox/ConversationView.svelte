<script lang="ts">
  ...
  let switchInteracted = $state(false);
  ...
  let widgetRoot = $state<HTMLDivElement | null>(null);
  ...
  onMount(() => {
    ...
    if (reducedMotion) {
      shown = realMode ? REAL_SESSION_COSTS.length : legacyMessages.length;
      paused = true;
      return;
    }

    if (typeof IntersectionObserver === "undefined" || !widgetRoot) return;
...
<div class="widget toycard" class:compact bind:this={widgetRoot}>
{#if compact && !realMode}
...
{:else if realMode}
  <div class="toolbar">
    ...
    <div class="toolbar-actions"><button class="transport toy-btn" onclick={useTransport}>{transportLabel}</button></div>
  </div>
  ...
  <div class="stage">
    <div class="chat-shell">
          ...
          <button class="bubble round-trip" class:selected={realSelected === step} onclick={() => inspect(stepNumber(step) - 1)} title="Inspect this API round trip">
            ...
            <span class="bubble-content">
              ...
              <span class="cost-cluster">
                <strong class="price toy-num" class:pricey={step.costWarm > realPriceThreshold} class:cheap={step.costWarm <= realPriceThreshold}>{exactMoney(step.costWarm)}</strong>
      ...
      <div class="scroll-controls" aria-label="Message list controls">
        <button class="scroll-button toy-btn" onclick={() => scrollMessages(-1)} disabled={!canScrollUp} aria-label="Scroll messages up">▲</button>
        <button class="scroll-button toy-btn" onclick={() => scrollMessages(1)} disabled={!canScrollDown} aria-label="Scroll messages down">▼</button>
      </div>
    </div>
  ...
  <div class="toolbar">
    {#if hasToggle}
      <div class="switch-wrap">
        ...
        <button class="switch switch-flex" class:on={leverOn} class:interacted={switchInteracted} onclick={flip} aria-label={`Switch to ${leverOn ? offLabel : onLabel}`}><i></i></button>
        ...
        {#each legacyVisible as message (message.id)}
          <button class="bubble {changeClass(message.id, message.usd)}" class:user={message.role === "user"} class:selected={selectedId === message.id} onclick={() => inspectLegacy(message.id)} title="Inspect this message's cost">
            ...
            <span class="bubble-content">
              ...
              <span class="cost-cluster">
                <strong class="price toy-num" class:pricey={message.usd > legacyPriceThreshold} class:cheap={message.usd <= legacyPriceThreshold}>{legacyMoney(message.usd)}</strong>
...
<style>
  ...
  .prompt-stack{display:flex;flex-direction:column;gap:4px;margin:8px 0 10px}.prompt-segment{display:grid;grid-template-columns:minmax(0,1fr) auto auto;gap:8px;align-items:center;padding:5px 7px;border:1px solid var(--toy-ink);border-radius:7px;font-size:.66rem;font-variant-numeric:tabular-nums}.prompt-segment span{font-weight:850;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.prompt-segment.read{background:var(--toy-green-soft);border-color:var(--toy-green-ink)}.prompt-segment.write{background:var(--toy-red-soft);border-color:var(--toy-red-ink)}.prompt-segment.input{background:var(--toy-gold-soft);border-color:var(--toy-gold-ink)}.prompt-segment small{min-width:90px;text-align:right}.cache-break{border-top:3px solid var(--toy-red-ink);color:var(--toy-red-ink);font-size:.61rem;font-weight:950;padding:3px 3px 0;text-transform:uppercase}.receipt-dt{border-top:2px solid var(--toy-ink);padding-top:5px!important;font-weight:900}
  .widget{height:100%;display:flex;flex-direction:column;border:2px solid var(--toy-ink);border-radius:20px 17px 22px 16px;overflow:hidden;background:var(--toy-paper);box-shadow:7px 8px 0 var(--toy-shadow);color:var(--toy-ink);min-height:0}button{font:inherit;color:inherit}.toolbar{position:relative;z-index:2;display:flex;justify-content:space-between;gap:12px;align-items:center;padding:12px 16px;background:var(--toy-cream-2);border-bottom:2px solid var(--toy-ink);flex:none}.live{font-size:.78rem;font-weight:900;text-transform:uppercase;letter-spacing:.06em}.live i{display:inline-block;width:8px;height:8px;border-radius:50%;background:var(--toy-green);margin-right:7px;box-shadow:0 0 0 4px color-mix(in srgb,var(--toy-green) 18%,transparent)}.switch-wrap{display:flex;align-items:center;gap:9px;font-size:.84rem;font-weight:800}.switch-wrap>span{opacity:.42}.switch-wrap>span.active{opacity:1}.switch{--switch-inset:3px;--switch-knob:19px;--switch-travel:calc(48px - 4px - var(--switch-inset) - var(--switch-inset) - var(--switch-knob));box-sizing:border-box;display:flex;align-items:center;width:48px;height:27px;padding:var(--switch-inset);border:2px solid var(--toy-ink);border-radius:99px;background:var(--toy-red);cursor:pointer;transition:background-color .22s}.switch.on{background:var(--toy-green)}.switch i{box-sizing:border-box;display:block;flex:0 0 var(--switch-knob);width:var(--switch-knob);height:var(--switch-knob);border-radius:50%;background:var(--toy-paper);border:1px solid var(--toy-ink)}.switch.on i{margin-left:auto}.switch.interacted.on i{animation:switch-on .22s ease-out}.switch.interacted:not(.on) i{animation:switch-off .22s ease-out}@keyframes switch-on{from{transform:translateX(calc(-1 * var(--switch-travel)))}}@keyframes switch-off{from{transform:translateX(var(--switch-travel))}}.transport{border:1.5px solid var(--toy-ink);background:var(--toy-paper);border-radius:9px;padding:6px 10px;font-weight:800;cursor:pointer;box-shadow:2px 2px 0 var(--toy-ink);white-space:nowrap}.timeline{height:76px;margin:0 20px;position:relative;overflow:hidden;flex:none}.rail{position:absolute;left:5%;right:5%;top:32px;height:3px;background:var(--toy-dash)}.tick{position:absolute;top:18px;transform:translateX(-50%) scale(.2);border:2px solid var(--toy-ink);width:30px;height:30px;border-radius:50%;background:var(--toy-shadow);opacity:0;cursor:pointer;transition:.35s cubic-bezier(.2,.9,.25,1.3);padding:0}.tick:disabled{pointer-events:none}.tick.arrived{opacity:1;transform:translateX(-50%) scale(1)}.tick.warm{background:var(--toy-green)}.tick.cold{background:var(--toy-red)}.tick b{font-size:.75rem}.tick small{position:absolute;top:32px;left:50%;transform:translateX(-50%);white-space:nowrap;font-size:.56rem;font-weight:700}.stage{min-height:190px;min-width:0;flex:1;display:flex;position:relative;padding:14px;background:var(--toy-paper);overflow:hidden}.chat{flex:1;min-width:0;min-height:0;max-height:100%;overflow:hidden}.chat{display:flex;flex-direction:column;gap:8px}.chat>.bubble:first-child{margin-top:auto}.bubble{display:block;max-width:82%;align-self:flex-start;border:1.5px solid var(--toy-ink);border-radius:6px 15px 15px;padding:8px 11px;background:var(--toy-cream);text-align:left;cursor:pointer;animation:pop .25s ease-out}.bubble.round-trip:nth-child(even),.bubble.user{align-self:flex-end;background:var(--toy-blue-soft);border-radius:15px 6px 15px 15px}.bubble small{display:block;color:var(--toy-muted);font-size:.66rem;font-weight:700;margin-bottom:2px}@keyframes pop{from{opacity:0;transform:translateY(8px) scale(.96)}}@keyframes greenflash{40%{background:var(--toy-green-soft)}}@keyframes redflash{40%{background:var(--toy-red-soft)}}.mini{display:flex;height:9px;overflow:hidden;border-radius:9px;background:var(--toy-shadow)}.mini i{min-width:2px}.mini .read{background:var(--toy-green);flex:10}.mini .write{background:var(--toy-red);flex:2}.mini .output{background:var(--toy-blue);flex:1}.mini .prefix{background:var(--toy-red)}.mini .input{background:var(--toy-blue)}.mini .ping{background:var(--toy-gold)}.mini .compact-cost{background:var(--toy-blue)}.inspector{position:absolute;inset:10px;background:var(--toy-cream);border:2px solid var(--toy-ink);border-radius:13px;padding:14px;box-shadow:5px 6px 0 var(--toy-gold);overflow:hidden}.inspector-title{display:flex;justify-content:space-between;font-weight:900}.inspector-title button{border:0;background:none;font-size:1.4rem;cursor:pointer}.inspector p{font-size:.78rem;line-height:1.35;margin:8px 0}.legacy-inspector dl{display:grid;grid-template-columns:1fr auto auto;gap:5px 12px;margin:0;font-size:.76rem}.legacy-inspector dd{margin:0;font-variant-numeric:tabular-nums}.legacy-inspector dd:last-child{text-align:right;font-weight:800}.explanation{color:var(--toy-muted)}.chunk-legend{display:flex;justify-content:space-between;gap:8px;margin:5px 0 10px;font-size:.58rem;font-weight:700}.chunk-legend span{display:flex;align-items:center}.chunk-legend i{width:8px;height:8px;border-radius:2px;background:var(--toy-green);margin-right:4px}.chunk-legend span:last-child i{background:var(--toy-red)}.receipt-lines{font-size:.69rem;font-variant-numeric:tabular-nums}.receipt-lines>div{display:grid;grid-template-columns:minmax(105px,.7fr) minmax(0,1.3fr);gap:8px;padding:3px 0;border-bottom:1px dashed var(--toy-dash)}.receipt-lines>div span:last-child{text-align:right}.receipt-total{border-bottom:0!important;border-top:2px solid var(--toy-ink);margin-top:3px;padding-top:6px!important;font-size:.77rem;font-weight:900}.receipt-total strong{text-align:right}.step-saving{text-align:right;font-weight:850;color:var(--toy-green-ink)}footer{display:flex;justify-content:space-between;gap:12px;align-items:center;padding:10px 16px;background:var(--toy-ink);color:var(--toy-paper);font-size:.7rem;flex:none}footer>span{text-transform:uppercase;letter-spacing:.05em;color:var(--toy-muted)}footer strong{font-size:.92rem;font-variant-numeric:tabular-nums;text-align:right}footer small{font-size:.63rem;color:var(--toy-muted)}footer em{font-style:normal;color:var(--toy-gold);margin:0 3px}.legacy-footer b{font-size:1rem;font-variant-numeric:tabular-nums}.day-total{margin-left:auto}.day-total b{color:var(--toy-gold)}
