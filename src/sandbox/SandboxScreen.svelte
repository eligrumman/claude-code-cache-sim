<script lang="ts">
  ...
  let customAfter = $state<SandboxConfig>({ ...PERSONAS.developer.defaults });
  ...
  const isModified = $derived(Object.keys(baselineConfig).some((key) => customAfter[key as keyof SandboxConfig] !== baselineConfig[key as keyof SandboxConfig]));
  ...
  function choosePersona(id: PersonaId) {
    ...
    spotlight = "keepWarm";
    showAfter = false;
    customAfter = { ...PERSONAS[id].defaults };
  }
  ...
  function changeLever(lever: LeverId, patch: Partial<SandboxConfig>) {
    spotlight = lever;
    customAfter = { ...customAfter, ...patch };
    showAfter = true;
  }

  function resetDefaults() {
    customAfter = { ...baselineConfig };
    showAfter = false;
  }
...
<main class="sandbox">
  ...
  <section class="paper breakdown-section" aria-labelledby="breakdown-title">
    ...
    <aside class="config-panel" class:open={configOpen} aria-labelledby="config-title">
      ...
      <p class="config-intro">Every change updates the field lab instantly.</p>
    ...
    <div class="lever-grid">
      {#each leverOrder as lever}
        <article class:active={spotlight === lever}>
          ...
          {#if lever === "subagents"}
            <div class="choice"><button class:chosen={customAfter.subagents === "same"} onclick={() => changeLever(lever, { subagents: "same" })}>same prompt</button><button class:chosen={customAfter.subagents === "different"} onclick={() => changeLever(lever, { subagents: "different" })}>different</button></div>
          {:else if lever === "ttl"}
            <div class="choice"><button class:chosen={customAfter.ttl === "5m"} onclick={() => changeLever(lever, { ttl: "5m" })}>5 min</button><button class:chosen={customAfter.ttl === "1h"} onclick={() => changeLever(lever, { ttl: "1h" })}>1 hour</button></div>
          {:else if lever === "context"}
            <label class="slider"><span>small</span><input aria-label="Context size" type="range" min="0.55" max="1.8" step="0.05" value={customAfter.context} oninput={(e) => changeLever(lever, { context: Number(e.currentTarget.value) })}><span>large</span></label>
          {:else if lever === "approval"}
            <div class="choice"><button class:chosen={customAfter.approval === "manual"} onclick={() => changeLever(lever, { approval: "manual" })}>manual</button><button class:chosen={customAfter.approval === "auto"} onclick={() => changeLever(lever, { approval: "auto" })}>auto</button></div>
          {:else if lever === "keepWarm"}
            <div class="choice"><button class:chosen={!customAfter.keepWarm} onclick={() => changeLever(lever, { keepWarm: false })}>off</button><button class:chosen={customAfter.keepWarm} onclick={() => changeLever(lever, { keepWarm: true })}>on</button></div>
          {:else}
            <div class="choice three">{#each ["sonnet", "opus", "fable"] as model}<button class:chosen={customAfter.model === model} onclick={() => changeLever(lever, { model: model as Model })}>{model}</button>{/each}</div>
          {/if}
        </article>
      {/each}
    </div>
    <p class="tiny-note">Delta badges compare each lever's suggested opposite with this persona's defaults.</p>
    </aside>
  </div>
</main>
...
<style>
  ...
  .sandbox { --red:#d94835; --green:#178353; --ink:#24231f; --muted:#6b6960; max-width: 1540px; margin: auto; padding: 24px 24px 100px; font-family: ui-rounded, "Arial Rounded MT Bold", system-ui, sans-serif; }
  ...
  .config-toggle { display:none; border:2px solid #24231f; background:#fff9df; border-radius:12px; padding:9px 13px; font-weight:900; cursor:pointer; box-shadow:3px 3px 0 #24231f; }
  ...
  .workspace { display:grid; grid-template-columns:minmax(0,1fr) 350px; align-items:start; gap:28px; }
  .visualization { min-width:0; }
  .personas { display:grid; grid-template-columns:1fr 1fr; gap:8px; }
  .personas button { background:white; border:2px solid #d9d5ca; border-radius:14px 12px 15px 11px; padding:11px 9px; color:var(--ink); cursor:pointer; transition:.25s cubic-bezier(.2,.8,.2,1); text-align:left; }
  ...
  .personas button>span { font-size:1.35rem; display:block; margin-bottom:4px; }
  .personas b { display:block; font-size:.75rem; line-height:1.15; }
  ...
  .config-panel { position:sticky; top:18px; max-height:calc(100vh - 36px); overflow:auto; overscroll-behavior:contain; border:2px solid #d9d4c7; border-radius:24px 20px 25px 19px; padding:22px; background:#fff; box-shadow:7px 8px 0 #f3cc4f, 0 18px 55px #44371812; z-index:20; }
  .config-head { display:flex; align-items:flex-start; justify-content:space-between; gap:12px; }
  .config-head h2 { margin:4px 0 0; font-size:1.55rem; letter-spacing:-.03em; }
  .config-intro { color:var(--muted); font-size:.82rem; line-height:1.4; margin:8px 0 20px; }
  .drawer-close { display:none; border:0; background:#f3efe5; width:42px; height:42px; border-radius:50%; font-weight:900; cursor:pointer; }
  .persona-control { border-block:1px dashed #d8d2c5; padding:16px 0; }
  .persona-control h3 { margin:0 0 10px; font-size:.8rem; text-transform:uppercase; letter-spacing:.1em; }
  .config-status { display:flex; align-items:center; justify-content:space-between; gap:10px; padding:15px 0 12px; }
  .config-status span { color:#77736b; font-size:.7rem; font-weight:900; }
  .config-status span.modified { color:#9a6800; }
  .reset { border:0; background:transparent; color:#5e5b53; padding:5px; font-size:.72rem; font-weight:850; text-decoration:underline; text-underline-offset:3px; cursor:pointer; }
  .reset:disabled { opacity:.35; cursor:default; }
  .lever-grid{display:grid;grid-template-columns:1fr;gap:10px}.lever-grid article{position:relative;border:2px solid #ded9cc;border-radius:15px 13px 17px 14px;padding:13px;min-height:105px;transition:.25s}.lever-grid article.active{border-color:#24231f;box-shadow:3px 4px 0 #f3cc4f;transform:translateY(-1px)}.lever-title{border:0;background:transparent;padding:0;font-weight:900;cursor:pointer;color:#24231f}.lever-title span{margin-right:6px}.delta{position:absolute;right:11px;top:12px;font-size:.61rem;font-weight:900;color:#777}.delta.positive{color:var(--red)}.delta.negative{color:var(--green)}.choice{display:flex;margin-top:20px}.choice button{flex:1;border:1px solid #cfc9bc;background:#faf8f2;padding:8px 4px;cursor:pointer;font-size:.69rem;font-weight:800}.choice button:first-child{border-radius:9px 0 0 9px}.choice button:last-child{border-radius:0 9px 9px 0}.choice button+button{border-left:0}.choice button.chosen{background:#24231f;color:white}.choice.three button:not(:first-child):not(:last-child){border-radius:0}.slider{display:flex;align-items:center;gap:8px;margin-top:23px;color:#77736a;font-size:.67rem;font-weight:800}.slider input{min-width:0;width:100%;accent-color:#e9573f}.tiny-note{color:#77736b;font-size:.68rem;line-height:1.4;margin:18px 0 0;border-top:1px dashed #ddd7cb;padding-top:13px}.scrim{display:none}
  @media(max-width:1020px){
    .sandbox { max-width:1180px; }
    nav { position:relative; z-index:31; }
    .scribble { margin-left:auto; margin-right:12px; }
    .config-toggle { display:block; }
    .workspace { display:block; }
    .config-panel { position:fixed; top:0; right:0; bottom:0; width:min(88vw,390px); max-height:none; border-radius:26px 0 0 22px; padding:22px 20px max(28px,env(safe-area-inset-bottom)); transform:translateX(calc(100% + 12px)); visibility:hidden; transition:transform .3s cubic-bezier(.2,.8,.2,1),visibility .3s; z-index:40; box-shadow:-12px 0 45px #24231f26; }
    .config-panel.open { transform:translateX(0); visibility:visible; }
    .drawer-close { display:block; flex:0 0 auto; }
    .scrim { display:block; position:fixed; z-index:35; inset:0; padding:0; border:0; background:#24231f73; backdrop-filter:blur(2px); cursor:pointer; }
    .spotlight-head,.section-head{flex-direction:column}.ab{width:100%}.ab button{flex:1}.timeline-band{overflow-x:auto}.session{min-width:150px}
  }
  @media(max-width:800px){.breakdown-grid{grid-template-columns:1fr}}
  @media(max-width:520px){.sandbox{padding:16px 14px 70px}header{padding:50px 0 36px}.money-row{gap:4px}.paper{padding:22px 16px}.breakdown-grid{gap:25px}.bucket-row{grid-template-columns:12px 115px 1fr 55px}.scribble{display:none}.spotlight{padding:20px 16px}.takeaway{font-size:.95rem}.config-toggle span{display:none}}
