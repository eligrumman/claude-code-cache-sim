<script lang="ts">
  ...
  import {
    ...
    type LeverId, type PersonaId, type SandboxConfig, type ScriptedMessage,
  } from "./model.js";
  ...
  const isModified = $derived(Object.keys(persona.defaults).some((key) => config[key as keyof SandboxConfig] !== persona.defaults[key as keyof SandboxConfig]));
  const ledgerOptions = $derived({
    ttl: config.ttl, model: config.model, prefixTok: result.prefixTok,
    workInTok: persona.workInTok, outputTok: persona.outputTok, keepWarm: config.keepWarm,
  });
  ...
  const conversationScript = $derived.by((): ScriptedMessage[] => {
  ...
  function resetDefaults() { config = { ...persona.defaults }; }
  function alternativeDelta(lever: LeverId) {
    return simulateDay(persona, configForLever(persona.defaults, lever, true)).totalUsd - baseline.totalUsd;
  }
...
<main class="sandbox">
  ...
  <section class="playground">
    <nav>
      ...
      <span class="persona-pill">{persona.emoji} {persona.name}</span>
    ...
    <div class="canvas" class:good={tone === "good"} class:bad={tone === "bad"}>
      <ConversationView script={conversationScript} options={ledgerOptions} bucketTotals={result.buckets} projectedTotal={result.totalUsd} compact />
    </div>
