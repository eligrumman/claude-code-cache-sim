...
| Article micro: Spend breakdown | none — modeled by `src/article/spendModel.ts` (engine-derived from Token Optimizer published multipliers) | **Modeled, not captured traffic.** Token Optimizer supplies the sourced 80%+ cache-read-volume and ~74% hit-rate framing; all dollar figures use this app's pricing engine. |
...
| Article macro: Macro Lab | none — modeled by `src/article/MacroLabWidget`, engine-derived; compaction effect is a model of prefix-shrink on cache-reads, not a measured capture | **Modeled, not captured traffic.** Token Optimizer supplies the sourced 80%+ cache-read-volume and ~74% hit-rate framing; task vocabulary comes from `MACRO_ROUTES`, and all dollar figures use the shared pricing engine. |
